...
export const projects: Project[] = [
  ...
  },
  {
    ...
    timeframe: "2025 — 至今",
    role: "AI 产品经理",
    summary: "把高校、基金会、项目、协议与资金数据从线上留痕，升级为可查询、可溯源、可协作的经营辅助 Agent。",
    ...
    problem: "平台已经能管住高校、基金会、项目、协议、资金和联系人等数据，但客户经理要判断经营全貌，仍需在多个模块间反复查询、手工拼接。核心矛盾有四个：数据分散导致客户全貌获取成本高，经营判断依赖个人经验，资金和协议缺少主动预警，月报与拜访材料大量依赖人工搬运和校验。",
    approach: [
      { title: "数据治理与权限层", body: "围绕高校、基金会、项目、协议、账户、资金、联系人和沟通记录统一实体、字段口径、主键关联、角色权限与数据脱敏。" },
      { title: "查看写审能力层", body: "将经营问答、客户全景、机会提示、月报生成、拜访 Brief、材料审核与任务协同拆成可调用的 AI 能力单元。" },
      { title: "Agent 编排层", body: "用意图识别、任务规划、工具调用、结果校验、人工确认和动作创建组织多步任务，不让模型凭记忆生成关键数字。" },
      { title: "八维评测集", body: "围绕任务完成、工具选择、参数准确、执行链路、数据溯源、合规边界、拒答合理性和输出格式建立回归验证。" },
    ],
    decisions: [
      { title: "按风险划分人机协作", body: "数据查询和格式整理可自动完成；月报草稿、拜访 Brief 和机会提示需要人工确认；涉及资金决策、客户承诺和协议签署必须由人接管。" },
      { title: "按业务心智重写工具", body: "不把后端接口直接暴露给模型，而是按“客户是谁、在做什么项目、钱到哪里、协议什么状态、最近沟通什么”组织工具描述。" },
      { title: "上线前先做可复现评测", body: "每次调整 Prompt、工具描述或业务规则，都用固定样本回归验证，避免只依赖上线后的零散反馈判断效果。" },
    ],
    outcomes: ["AI 生成内容的分行采纳率从不足 50% 提升至约 70%", "Agent 工具调用准确率从 71% 提升至 89%", "单次迭代验证周期从约 2 周缩短到 3 天", "平台与业务团队共同支撑南京高校项目资金引流 6.5 亿元、北京高校定存中标 7000 万元"],
    reflection: ["企业级 Agent 先要把数据和权限做对，再谈生成能力", "工具调用准确率低，往往是工具定义不符合业务语言", "金融场景不能盲目追求全自动，信任来自边界、溯源和人工确认"],
